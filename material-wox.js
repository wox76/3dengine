...
import { WebGLMultipleRenderTargets, ShaderMaterial, GLSL3, NoBlending, Uniform, ShaderChunk, ShaderLib, UniformsUtils, Vector2, Matrix4, Vector3, Clock, Quaternion, LinearFilter, HalfFloatType, FramebufferTexture, RGBAFormat, WebGLRenderTarget, FloatType, DataTexture, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, MeshPhysicalMaterial, Color, NearestFilter, DepthTexture, TextureLoader, RepeatWrapping, NoColorSpace, RedFormat, Source, Matrix3, NoToneMapping, SRGBColorSpace } from 'three';
...
const createGlobalDisableIblRadianceUniform = () => {
  ...
  const originalClone = UniformsUtils.clone;

  UniformsUtils.clone = uniforms => {
    const result = originalClone(uniforms);

    if (result.iblRadianceDisabled) {
      Object.defineProperty(result.iblRadianceDisabled, "value", {
        get() {
          if (!globalIblRadianceDisabledUniform.value) return false;
          const isTransparent = result.opacity && result.opacity.value < 1.0;
          const isTransmissive = result.transmission && result.transmission.value > 0.0;
          return !(isTransparent || isTransmissive);
        },

        set(v) {// allow setting if needed, or ignore
        },
...
const createGlobalDisableIblIradianceUniform = () => {
  ...
  UniformsUtils.clone = uniforms => {
    ...
    if (result.iblIrradianceDisabled) {
      Object.defineProperty(result.iblIrradianceDisabled, "value", {
        get() {
          if (!globalIblIrradianceDisabledUniform.value) return false;
          const isTransparent = result.opacity && result.opacity.value < 1.0;
          const isTransmissive = result.transmission && result.transmission.value > 0.0;
          return !(isTransparent || isTransmissive);
        },
...
const isChildMaterialRenderable = (c, material = c.material) => {
  const isTransparentOrTransmissive = material.transparent || material.transmission !== undefined && material.transmission > 0 || material.opacity !== undefined && material.opacity < 1.0;
  return material.visible && material.depthWrite && material.depthTest && !isTransparentOrTransmissive && !isGroundProjectedEnv(c);
};
...
class GBufferMaterial extends MeshPhysicalMaterial {
  onBeforeCompile(shader) {
    ...
    shader.uniforms.blueNoiseTexture = {
      value: null
    };
    shader.uniforms.blueNoiseRepeat = {
      value: new Vector2(1, 1)
    };
    shader.uniforms.resolution = {
      value: new Vector2(1, 1)
    };
    shader.uniforms.cameraMoved = {
      value: false
    };
    const vertexShader = shader.vertexShader.replace("void main() {",
    /* glsl */
    `
            ...
            `);
    shader.vertexShader = vertexShader; // delete all includes that have the pattern "#include <lights_something>"
    ...
    shader.fragmentShader = shader.fragmentShader.replace(/#include <lightmap.*>/g, ""); // remove opaque_fragment include
...
props = props.filter(key => !key.startsWith("_") && !key.startsWith("is") && key !== "uuid" && key !== "type"); // this function attempts to copy all the props from the original material to the new GBufferMaterial
...
const propsPrimitive = props.filter(key => typeof gBufferMaterial[key] === "string" || typeof gBufferMaterial[key] === "number" || typeof gBufferMaterial[key] === "boolean");
function copyPropsToGBufferMaterial(originalMaterial, gBufferMaterial) {
  for (const prop of propsPrimitive) {
    if (gBufferMaterial[prop] !== originalMaterial[prop]) {
...
class GBufferPass extends Pass {
  ...
  setGBufferMaterialInScene() {
    this.visibleMeshes = getVisibleChildren$1(this._scene); // const cameraMoved = didCameraMove(this._camera, this.lastCameraPosition, this.lastCameraQuaternion)
...
const useBlueNoise = material => {
  material.uniforms.blueNoiseTexture = {
    value: blueNoiseTexture
  };
  material.uniforms.blueNoiseSize = {
  ...
  material.fragmentShader = material.fragmentShader.replace("uniform vec2 resolution;", "uniform vec2 resolution;\n" + blue_noise);
...
class SSGIMaterial extends ShaderMaterial {
  constructor() {
    super({
      ...
      fragmentShader: fragmentShader$3.replace("#include <ssgi_utils>", ssgi_utils).replace("#include <gbuffer_packing>", gbuffer_packing),
      vertexShader,
      blending: NoBlending,
      depthWrite: false,
      depthTest: false,
...
var fragmentShader$2 = "#define GLSLIFY 1\nvarying vec2 vUv;uniform sampler2D inputTexture;uniform sampler2D inputTexture2;uniform sampler2D depthTexture;uniform sampler2D normalTexture;uniform mat4 projectionMatrix;uniform mat4 projectionMatrixInverse;uniform mat4 cameraMatrixWorld;uniform float radius;uniform float phi;uniform float lumaPhi;uniform float depthPhi;uniform float normalPhi;uniform float roughnessPhi;uniform float specularPhi;uniform vec2 resolution;layout(location=0)out vec4 gOutput0;layout(location=1)out vec4 gOutput1;\n#include <common>\n#include <gbuffer_packing>\n#define luminance(a) pow(dot(vec3(0.2125, 0.7154, 0.0721), a), 0.125);\nvec3 getWorldPos(float depth,vec2 coord){float z=depth*2.0-1.0;vec4 clipSpacePosition=vec4(coord*2.0-1.0,z,1.0);vec4 viewSpacePosition=projectionMatrixInverse*clipSpacePosition;vec4 worldSpacePosition=cameraMatrixWorld*viewSpacePosition;worldSpacePosition.xyz/=worldSpacePosition.w;return worldSpacePosition.xyz;}float distToPlane(const vec3 worldPos,const vec3 neighborWorldPos,const vec3 worldNormal){vec3 toCurrent=worldPos-neighborWorldPos;float distToPlane=abs(dot(toCurrent,worldNormal));return distToPlane;}vec2 viewSpaceToScreenSpace(const vec3 position){vec4 projectedCoord=projectionMatrix*vec4(position,1.0);projectedCoord.xy/=projectedCoord.w;projectedCoord.xy=projectedCoord.xy*0.5+0.5;return projectedCoord.xy;}void toDenoiseSpace(inout vec3 color){color=log(color+1.);}void toLinearSpace(inout vec3 color){color=exp(color)-1.;}void evaluateNeighbor(const vec4 neighborTexel,inout vec3 denoised,inout float totalWeight,const float wBasic){float w=min(1.,wBasic);denoised+=w*neighborTexel.rgb;totalWeight+=w;}Material centerMat;vec3 normal;float centerDepth;vec3 centerWorldPos;float centerLumDiffuse;float centerLumSpecular;float specularFactor;float w,w2;struct Neighbor{vec4 texel;float weight;};Neighbor getNeighborWeight(vec2 neighborUv,bool isDiffuseGi){float neighborDepth=textureLod(depthTexture,neighborUv,0.0).r;if(neighborDepth==1.0)return Neighbor(vec4(0.),0.);float distanceToCenter=length(vUv-neighborUv)+1.;distanceToCenter=pow(distanceToCenter,8.)-1.;vec4 neighborDiffuseGi=textureLod(inputTexture,neighborUv,0.);vec4 neighborSpecularGi=textureLod(inputTexture2,neighborUv,0.);toDenoiseSpace(neighborDiffuseGi.rgb);toDenoiseSpace(neighborSpecularGi.rgb);float neighborLuminance=luminance(neighborDiffuseGi.rgb);float neighborLuminance2=luminance(neighborSpecularGi.rgb);\n#ifdef GBUFFER_TEXTURE\nMaterial neighborMat=getMaterial(gBufferTexture,neighborUv);vec3 neighborNormal=neighborMat.normal;\n#else\nvec3 neighborDepthVelocityTexel=textureLod(normalTexture,neighborUv,0.).xyz;vec3 neighborNormal=unpackNormal(neighborDepthVelocityTexel.b);\n#endif\nvec3 neighborWorldPos=getWorldPos(neighborDepth,neighborUv);float normalDiff=1.-max(dot(normal,neighborNormal),0.);float depthDiff=10.*distToPlane(centerWorldPos,neighborWorldPos,centerMat.normal);float lumaDiff=mix(abs(centerLumDiffuse-neighborLuminance),0.,w);float lumaDiff2=mix(abs(centerLumSpecular-neighborLuminance2),0.,w2);\n#ifdef GBUFFER_TEXTURE\nfloat roughnessDiff=abs(centerMat.roughness-neighborMat.roughness);float wBasic=exp(-normalDiff*normalPhi-depthDiff*depthPhi-roughnessDiff*roughnessPhi);\n#else\nfloat wBasic=exp(-normalDiff*normalPhi-depthDiff*depthPhi);\n#endif\nif(isDiffuseGi){wBasic=mix(wBasic,exp(-normalDiff*10.),w);float wDiff=w*pow(wBasic*exp(-lumaDiff*lumaPhi),phi/w);wDiff=min(wDiff,1.);return Neighbor(neighborDiffuseGi,wDiff);}else{wBasic=mix(wBasic,exp(-normalDiff*10.),w2);float wSpec=w2*pow(wBasic*exp(-lumaDiff2*lumaPhi),phi/w2);wSpec=pow(wSpec,1.+specularPhi*specularFactor);wSpec=min(wSpec,1.);return Neighbor(neighborSpecularGi,wSpec);}}const vec2 poissonDisk[8]=POISSON_DISK_SAMPLES;void main(){centerDepth=textureLod(depthTexture,vUv,0.).r;if(centerDepth==1.0){discard;return;}vec4 centerDiffuseGi=textureLod(inputTexture,vUv,0.0);vec4 centerSpecularGi=textureLod(inputTexture2,vUv,0.0);float a=centerDiffuseGi.a;float a2=centerSpecularGi.a;if(a+a2>512.){discard;return;}w=1./sqrt(a+1.);w2=1./sqrt(a2+1.);toDenoiseSpace(centerDiffuseGi.rgb);toDenoiseSpace(centerSpecularGi.rgb);centerLumDiffuse=luminance(centerDiffuseGi.rgb);centerLumSpecular=luminance(centerSpecularGi.rgb);centerMat=getMaterial(gBufferTexture,vUv);\n#ifdef GBUFFER_TEXTURE\nnormal=centerMat.normal;\n#else\nvec3 depthVelocityTexel=textureLod(normalTexture,vUv,0.).xyz;normal=unpackNormal(depthVelocityTexel.b);\n#endif\nspecularFactor=1.-centerMat.roughness;specularFactor*=specularFactor;float totalWeight=1.;float totalWeight2=1.;vec3 denoisedDiffuse=centerDiffuseGi.rgb;vec3 denoisedSpecular=centerSpecularGi.rgb;centerWorldPos=getWorldPos(centerDepth,vUv);vec3 cameraPos=cameraMatrixWorld[3].xyz;float distanceToCamera=distance(centerWorldPos,cameraPos);float roughnessRadius=mix(sqrt(centerMat.roughness),1.,0.5*(1.-centerMat.metalness));vec4 random=blueNoise();float r=sqrt(random.a)*exp(-(a+a2)*0.01)*radius*roughnessRadius;float angle=random.r*2.*PI;float s=sin(angle),c=cos(angle);mat2 rm=mat2(c,-s,s,c);rm*=r*25.0/distanceToCamera;for(int i=0;i<8;i++){vec2 offset=rm*poissonDisk[i];vec2 neighborUv=vUv+offset;Neighbor neighborDiffuse=getNeighborWeight(neighborUv,true);Neighbor neighborSpecular=getNeighborWeight(neighborUv,false);evaluateNeighbor(neighborDiffuse.texel,denoisedDiffuse,totalWeight,neighborDiffuse.weight);evaluateNeighbor(neighborSpecular.texel,denoisedSpecular,totalWeight2,neighborSpecular.weight);}denoisedDiffuse/=totalWeight;denoisedSpecular/=totalWeight2;toLinearSpace(denoisedDiffuse);toLinearSpace(denoisedSpecular);gOutput0=vec4(denoisedDiffuse,centerDiffuseGi.a);gOutput1=vec4(denoisedSpecular,centerSpecularGi.a);}"; // eslint-disable-line

function generateDenoiseSamples(texelSize) {
  const sqr2 = 2 ** 0.5; // by Nvidia ReBLUR, for distribution see: https://www.desmos.com/calculator/abaqyvswem

  let samples = [new Vector2(-1, 0), new Vector2(0, -1), new Vector2(1, 0), new Vector2(0, 1), new Vector2(-0.25 * sqr2, -0.25 * sqr2), new Vector2(0.25 * sqr2, -0.25 * sqr2), new Vector2(0.25 * sqr2, 0.25 * sqr2), new Vector2(-0.25 * sqr2, 0.25 * sqr2)];
...
/* eslint-disable camelcase */
const finalFragmentShader$1 = fragmentShader$2.replace("#include <gbuffer_packing>", gbuffer_packing);
...
class PoissionDenoisePass extends Pass {
  constructor(camera, textures, options = defaultPoissonBlurOptions) {
    ...
    this.renderTargetB = new WebGLMultipleRenderTargets(1, 1, 2, renderTargetOptions); // give the textures of renderTargetA and renderTargetB names
...
class Denoiser {
  constructor(scene, camera, texture, options = defaultDenosierOptions) {
    ...
    if (options.denoiseMode === "full" || options.denoiseMode === "full_temporal") {
      ...
      this.denoiserComposePass = new DenoiserComposePass(camera, composerInputTextures, options.gBufferPass.texture, options.gBufferPass.renderTarget.depthTexture);
    }
  }
  ...
  setSize(width, height) {
    var _this$denoisePass, _this$denoiserCompose;

    this.velocityDepthNormalPass.setSize(width, height);
    this.temporalReprojectPass.setSize(width, height);
    (_this$denoisePass = this.denoisePass) == null ? void 0 : _this$denoisePass.setSize(width, height);
    (_this$denoiserCompose = this.denoiserComposePass) == null ? void 0 : _this$denoiserCompose.setSize(width, height);
  }

  dispose() {
    var _this$denoisePass2, _this$denoiserCompose2;

    this.velocityDepthNormalPass.dispose();
    this.temporalReprojectPass.dispose();
    (_this$denoisePass2 = this.denoisePass) == null ? void 0 : _this$denoisePass2.dispose();
    (_this$denoiserCompose2 = this.denoiserComposePass) == null ? void 0 : _this$denoiserCompose2.dispose();
  }

  denoise(renderer) {
    var _this$denoisePass3, _this$denoiserCompose3;

    if (this.isOwnVelocityDepthNormalPass) this.velocityDepthNormalPass.render(renderer);
    this.temporalReprojectPass.render(renderer);
    (_this$denoisePass3 = this.denoisePass) == null ? void 0 : _this$denoisePass3.render(renderer);
    (_this$denoiserCompose3 = this.denoiserComposePass) == null ? void 0 : _this$denoiserCompose3.render(renderer);
  }
...
class SSGIEffect extends Effect {
  constructor(composer, scene, camera, options) {
    ...
    super("SSGIEffect", ssgi_compose, {
      ...
      uniforms: new Map([["inputTexture", new Uniform(null)], ["sceneTexture", new Uniform(null)], ["depthTexture", new Uniform(null)], ["isDebug", new Uniform(false)], ["toneMapping", new Uniform(NoToneMapping)], ["ssgiMix", new Uniform(1.0)]])
    });
...
class MotionBlurEffect extends Effect {
  constructor(velocityPass, options = defaultOptions$1) {
    ...
    super("MotionBlurEffect", motion_blur, {
      ...
      uniforms: new Map([["inputTexture", new Uniform(null)], ["velocityTexture", new Uniform(velocityPass.texture)], ["blueNoiseTexture", new Uniform(null)], ["blueNoiseSize", new Uniform(new Vector2())], ["resolution", new Uniform(new Vector2())], ["intensity", new Uniform(1)], ["jitter", new Uniform(1)], ["frame", new Uniform(0)], ["deltaTime", new Uniform(0)]]),
...
class TAAPass extends Pass {
  ...
  render(renderer, inputBuffer) {
    this.frame = (this.frame + 1) % 4096;
    this.fullscreenMaterial.uniforms.inputTexture.value = inputBuffer.texture; // check if the camera has moved by comparing the camera's world matrix and projection matrix
...
export { HBAOEffect, MotionBlurEffect, PoissionDenoisePass, SSGIEffect, SSREffect, SharpnessEffect, TAAPass, TRAAEffect, TemporalReprojectPass, VelocityDepthNormalPass, VelocityPass };
